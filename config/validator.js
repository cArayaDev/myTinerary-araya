...
const validator = (req, res, next) => {
    ...
    const schema = joi.object({
        name: joi.string().max(20).min(3).trim().pattern(new RegExp('[a-zA-Z]')).messages({
            'string.empty':'This field is required',
            'string.min':'This field name must be at least 3 characters long'
        }),
        lastname: joi.string().max(20).min(3).trim().pattern(new RegExp('[a-zA-Z]')).messages({
            'string.empty':'This field is required',
        ...
        urlphoto: joi.string().max(250).min(8).trim().pattern(new RegExp('[a-zA-Z]')).messages({
            'string.empty':'This field is required',
        }),
        country: joi.string().max(30).min(3).trim().messages({
            'string.empty':'This field is required',
