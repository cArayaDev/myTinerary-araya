...
const validator = (req, res, next) => {
    ...
    const schema = joi.object({
        ...
        urlphoto: joi.string().max(100).min(8).trim().pattern(new RegExp('[a-zA-Z]')).required().messages({
            'string.empty':'This field is required',
        }),
        country: joi.string().max(30).min(3).trim().pattern(new RegExp('[a-zA-Z]')).required().messages({
            'string.empty':'This field is required',
